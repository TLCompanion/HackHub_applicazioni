...
public class CreaHackathonHandler {
     ...
     *
     * @param repositoryUtente    la repository per recuperare gli utenti che saranno organizzatori, mentori e giudici
     ...
     */
    public CreaHackathonHandler(RepositoryUtente repositoryUtente, RepositoryHackathon
    ...
    private void validazione(HackathonRequest request, String nomeUtente) {
        ...
        if (repositoryUtente.findByNomeUtente(request.nomeGiudice()).isEmpty() || request.nomeMentori().stream().
                anyMatch(nome -> repositoryUtente.findByNomeUtente(nome).isEmpty())) {
    ...
    private Map<Utente, RuoloStaff> gestisciStaff(List<String> nomiMentori, String nomeGiudice) {
        List<Utente> mentori = nomiMentori.stream().map(nome -> repositoryUtente.findByNomeUtente(nome).orElseThrow(() ->
                new NotFoundException("L'utente specificato non esiste: " + nome))).toList();
        Utente giudice = repositoryUtente.findByNomeUtente(nomeGiudice).orElseThrow(() ->
    ...
    private void gestisciOrganizzatore(String nomeUtente, Hackathon hackathon) {
        Utente organizzatore = repositoryUtente.findByNomeUtente(nomeUtente).orElseThrow(() ->
